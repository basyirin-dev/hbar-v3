...
\author{Supplementary Material for ``The H-Bar Model''}
...
This appendix contains the cognitive extensions to the core H-Bar
framework presented in the main paper. These extensions build upon
the three core state variables ($\delta_A$, $\beta_A$, $\sigma_A$)
and are not required for understanding the basic H-Bar system.
Readers seeking the minimal H-Bar framework need only study the
...
is consistent with the H-Bar optimal multi-step arc.
