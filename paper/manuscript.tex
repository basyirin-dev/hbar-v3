...
\hypersetup{
  pdftitle={The Σ-Model: Schema-Coherence Suppression as the Origin of Compositional Generalisation Failure},
  pdfauthor={Anonymous},
...
\title{The $\Sigma$-Model: Schema-Coherence Suppression as the Origin of Compositional Generalisation Failure --- A Dynamical-Systems Framework for Generalisation Failure}
...
(Eqs.~14,~28,~29) with local existence, invariance, and bifurcation
...
\item \textbf{Step 1 (Section~\ref{sec:core-framework}):} Define observables ($\delta_A$, $\beta_A$, $\sigma_A$) and show $\sigma_A$ is latent
  \begin{itemize}[nosep]
  \item Lemma~3.1: $\sigma_A$ cannot be inferred from $\delta_A$ alone [Proposition~3.6--3.7] % CLAIM:C-021
  \end{itemize}
...
% CLAIM:C-024
\textit{Define the timescale parameter $\epsilon = \max(\nu_M, \kappa_P, \kappa_I, \kappa_F) / \min(\eta_{\max}, \rho, \gamma) \ll 1$.
Decompose $\mathbf{x}_A = (\mathbf{x}_A^{\text{fast}}, \mathbf{x}_A^{\text{slow}})$
...
(see the companion proof-reconstruction document
Proposition~3.3 (see companion proof document) verifies that the Jacobian
\[
\mathbf{J}_{\text{fast}} = 
\begin{pmatrix}
...
\textbf{VRAM complexity.} The phenomenological implementation (Section~10.1)
...
Deterministic proxy fusion (Eqs.~7--9) & §3.1.4 & First-order Taylor approximation & Nonlinear fusion corrections at $k \gg 1$ & §12.4 Extensions \\
$\hat{\sigma}_A$ Gaussian estimator & Prop~3.6 & CLT over $N$ evaluation samples & Non-Gaussian tails for rare-event benchmarks & §12.3 Empirical calibration \\
Additive-multiplicative coupling forms & Eqs.~24--25 & Phenomenological, not first-principles & Could arise from product of more fundamental gating terms & §12.2(c) \\
Constant rate parameters ($\rho$, $\epsilon_\sigma$, $\gamma$, etc.) & All ODEs & Stationarity assumed over training horizon & Time-varying rates in practice; scheduled hyperparameters create drift & §12.4 Extensions \\
...
All experimental results in this paper are reproducible using the provided codebase. The main ODE validation experiment is implemented as a config-driven Jupyter notebook (\texttt{experiments/h-bar-experiment.ipynb}) whose parameters are defined in \texttt{experiments/configs/base.yaml}. A minimal reproducible example (\texttt{scripts/mre\_sigma.py}) runs in under 60~seconds on CPU and exercises the full ODE coupling, signal extraction, and loss modulation pipeline. The test suite (\texttt{tests/}; 31 tests) covers the ODE solver, config loading, and metric functions and is verified via \texttt{pytest} and a standalone smoke test (\texttt{scripts/smoke\_test.py}). All code uses deterministic seeding (\texttt{torch.manual\_seed(run\_id * 42 + 7)}, \texttt{cudnn.deterministic = True}) and is audited for deprecated APIs.
...
\caption{\textbf{Σ-Model Training Pipeline (Algorithm~3.2).}
...
\subsection*{A. All Articles}
% 5 items required for all submissions.
\begin{enumerate}
  \item \textbf{All claims investigated in this work are clearly stated.}
    \textbf{Yes.} See \S\ref{sec:predictions} (Nine Core Falsifiable Predictions),
    Proposition~4.1, and the enumerated contribution list in \S\ref{sec:introduction}.

  \item \textbf{Clear explanations are given for how the work reported substantiates the claims.}
    \textbf{Yes.} Each prediction in \S\ref{sec:predictions} is derived from the ODE
    system (Propositions~3.1--4.4) and tested in \S\ref{sec:ode-exp}
    with statistical analysis. Intuition blocks follow key equations to bridge
    formalism and interpretation.

  \item \textbf{Limitations or technical assumptions are stated clearly and explicitly.}
    \textbf{Yes.} Formal assumptions are stated in \S\ref{sec:state-space} and
    \S\ref{sec:equilibrium}. Failure-mode analysis bounds the framework's
    applicability (\S\ref{sec:scope},\S\ref{sec:open-questions}), and the
    assumption--boundary ledger enumerates 12 explicit limitations.

  \item \textbf{Conceptual outlines and/or pseudo-code descriptions of the AI methods
    introduced in this work are provided, and important implementation details are discussed.}
    \textbf{Yes.} Algorithms~3.1 (ODE integration) and~3.2 (adaptive curriculum)
    provide formal pseudo-code. The five-phase training arc is described in
    \S\ref{sec:phase-structure}. The \texttt{code/sigma/} package implements
    the full pipeline.

  \item \textbf{Motivation is provided for all design choices, including algorithms,
    implementation choices, parameters, data sets and experimental protocols beyond metrics.}
    \textbf{Yes.} Design choices are motivated throughout \S\ref{sec:math-foundations}
    and \S\ref{sec:phase-structure}. Hyperparameter selection is documented in
    \texttt{experiments/configs/base.yaml} with inline comments. The
    pre-registered protocol (\S\ref{sec:empirical-validation}) specifies
    statistical tests, sample sizes, and exclusion criteria.
\end{enumerate}

\subsection*{B. Theoretical Contributions}
\noindent\textit{Does this paper include theoretical contributions?} \textbf{Yes.}

\begin{enumerate}
  \item \textbf{All assumptions and restrictions are stated formally and clearly.}
    \textbf{Yes.} See Definition~3.1 (state space), Assumptions~3.1--3.5
    (differentiability, boundedness, coupling structure), and the
    assumption--boundary ledger in Appendix~C.

  \item \textbf{Novel claims are stated formally (e.g., in theorem statements).}
    \textbf{Yes.} Propositions~3.1--3.5 establish existence, uniqueness,
    invariance, and domain properties of the ODE system.
    Propositions~4.1--4.4 establish bifurcation results for the
    $\sigma_A$ dynamics.

  \item \textbf{Proofs of non-trivial claims are provided in sufficient detail to
    permit verification by readers with a reasonable level of expertise.}
    \textbf{Yes.} Proofs are provided in \S\ref{sec:math-foundations}--\ref{sec:bifurcation}
    with full calculations. The proof-reconstruction directory
    (\texttt{docs/proof-reconstruction/}) contains expanded derivations.

  \item \textbf{Complex formalism, such as definitions or proofs, is motivated and
    explained clearly.}
    \textbf{Yes.} Intuition blocks follow each major formal result (after
    \eqref{eq:depth-ode}, \eqref{eq:sigma-ode}, Proposition~4.1).
    The piecewise-smooth (Filippov) formulation is motivated by the
    max(0,~.) and |~.| non-smoothness in the coupling structure.

  \item \textbf{The use of mathematical notation serves the purpose of enhancing
    clarity, i.e., use that does not introduce unnecessary complexity.}
    \textbf{Yes.} Notation is introduced incrementally with natural-language
    glosses. Table~1 provides a notation summary. All symbols are defined
    at first use and collected in the notation index (\S\ref{sec:reliability}).

  \item \textbf{Appropriate citations are given for all non-trivial theoretical tools
    and techniques used.}
    \textbf{Yes.} Citations are provided for Filippov solutions
    (standard theory), IMEX-RK stability methods, bifurcation
    theory~\cite{kuznetsov1998}, and the ARS(2,3,2) tableau.
\end{enumerate}

\subsection*{C. Computational Experiments}
\noindent\textit{Does this paper include computational experiments?} \textbf{Yes.}

\begin{enumerate}
  \item \textbf{All source code required for conducting experiments is included in
    an online appendix or will be made publicly available upon publication.}
    \textbf{Yes.} Complete source code is available in the \texttt{code/}
    directory and archived in \texttt{code\_archive.tgz}. The reproducibility
    package includes docker-compose and a Makefile for one-command setup.

  \item \textbf{The source code comes with a license that allows usage for research purposes.}
    \textbf{Yes.} The repository is licensed under the MIT License
    (see \texttt{LICENSE} in the source root).

  \item \textbf{The method used for setting the seeds (or other source of randomness)
    is described, and the management of randomness is clearly explained.}
    \textbf{Yes.} Pseudorandom seeding is documented in
    \texttt{code/}\allowbreak\texttt{sigma/}\allowbreak\texttt{utils/}\allowbreak\texttt{seeding.py}
    and \S\ref{sec:empirical-validation}. All experiments use fixed seeds
    ($0$--$14$) with deterministic PyTorch operations.

  \item \textbf{The execution environment (operating system, hardware, software
    versions) is clearly described.}
    \textbf{Yes.} \texttt{HARDWARE.md} documents the GPU (NVIDIA Tesla T4),
    CPU, RAM, and CUDA version. \texttt{pyproject.toml} and
    \texttt{requirements-locked.txt} specify exact software versions.
    The Dockerfile (\texttt{Dockerfile}) provides a reproducible
    containerised environment.

  \item \textbf{The evaluation metrics are precisely defined, and the method used
    to compute them is described.}
    \textbf{Yes.} In-distribution and out-of-distribution accuracy are
    defined in \S\ref{sec:ode-exp} with the compositional gap metric.
    Cohen's $d$, Welch's $t$-test, and Bonferroni correction are
    specified. The code computing these metrics is in
    \texttt{code/sigma/benchmarks/}.

  \item \textbf{The number of algorithm runs used to compute results is specified,
    and the analysis goes beyond single-dimensional summaries to include
    statistics on variation.}
    \textbf{Yes.} $n = 15$ runs per condition (45 total) with Welch's
    $t$-tests, Cohen's $d$ effect sizes, standard deviations, and
    post-hoc power analysis ($>99.9\%$ at $\alpha = 0.00625$).
    Segmented regression with 95\% confidence intervals is used for
    Phase~2 inflection analysis.

  \item \textbf{All parameters are described, and (where applicable) the process
    of finding hyperparameters is described.}
    \textbf{Yes.} All hyperparameters are documented in
    \texttt{experiments/configs/base.yaml} with inline explanations.
    See also Table~4 (parameter values) in \S\ref{sec:ode-exp}.
    The hyperparameter search procedure is described in
    \S\ref{sec:ode-exp} (grid search over coupling strengths).
\end{enumerate}

\subsection*{D. Datasets}
\noindent\textit{Does this paper rely on one or more datasets?} \textbf{Yes.}

\begin{enumerate}
  \item \textbf{Datasets are publicly available or will be made publicly available.}
    \textbf{Yes.} The synthetic compositional benchmark is generated
    on-the-fly by \texttt{code/sigma/benchmarks/generator.py}. The
    generator code and configuration are included in the source archive.
    The \texttt{hackathon/} directory contains the Phase~0--5 task
    definitions.

  \item \textbf{Datasets are properly documented with metadata, and relevant
    analyses are provided.}
    \textbf{Yes.} Dataset statistics (train/OOD splits, phase composition)
    are reported in \S\ref{sec:ode-exp}. The benchmark generator supports
    full metadata export.

  \item \textbf{The method for dividing data into training, validation, and test
    sets is described.}
    \textbf{Yes.} The Phase~0--5 protocol defines fixed splits: Phase~0
    (12 templates, 144~instances each) for training; Phases~1--5 for
    OOD compositional evaluation. See \S\ref{sec:ode-exp} and
    \texttt{experiments/configs/base.yaml}.
\end{enumerate}
