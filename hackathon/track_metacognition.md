...
**Two-tier proxy architecture and calibration error dynamics (§3.1.3, §10.1).** Schema coherence σ_A is operationalised via a two-tier proxy architecture: Tier 1 (training-time) uses a causal intervention probe (Eq. 3c) or augmentation consistency (Eq. 3d) to provide per-checkpoint estimates; Tier 2 (evaluation-time) uses the Systematic Generalisation Gap (SGG, Eq. 3b) as ground-truth at evaluation checkpoints. The operative value during training is σ̃_A (training-time proxy), while σ̂_A (evaluation-time proxy) is used for validation. Calibration between tiers is managed via Appendix A.4's three-condition battery (ID, OOD-struct, OOD-surf-conflict). The calibration error ζ_A follows the ODE (Eq. 38a): ζ̇_A = -ν_M ζ_A - ξ_M Ω_AI (ζ_A + σ_A) - ρ P_A α_A (1 - σ_A) + ε_σ σ_A Ω_AI. The first term (-ν_M ζ_A) is a restoring force driving calibration error toward zero; the second term (-ξ_M Ω_AI (ζ_A + σ_A)) inflates overconfidence under AI bypass; the third and fourth terms couple to the schema coherence ODE (Eq. 28).

**Boundedness and steady-state of the self-model (§4.4.2).** The self-model M̂_A is bounded in [0,1] by Nagumo's theorem. Its steady-state (Eq. 39a) is M̂_A* = σ_A / (1 + Ω_AI/Π_7) ≤ σ_A. This means that sustained AI bypass (Ω_AI > 0) produces underconfidence at equilibrium (M̂_A* < σ_A), not sustained overconfidence. Overconfidence (ζ_A > 0) is transient and driven by the initial condition; the system converges to underconfidence under sustained Ω_AI.
...
- **ISSUE #7** [RESOLVED]: Phase 2 entry is now externally verifiable via Prediction 9 (§9) — the OOD acceleration signature in dAcc_OOD/dt. When H-PTB detects Phase 2 entry for a model, the H-MCB calibration protocol should be re-administered: the $ζ_A$ measurement becomes more critical post-Phase 2 because the $ν_M$ corrective term in the self-model ODE (Eq. 39) begins to dominate as $σ_A$ crosses $σ_{critical}$, producing calibration convergence. Re-measuring $ζ_A$ after Phase 2 entry tests whether the agent's self-model has updated to reflect its improved schema coherence.
